...
@section('page_subtitle', 'Kelola keseluruhan data kategori disini')

@section('content')

<div class="flex justify-between items-center mb-4">
    <form action="{{ route('admin.categories.index') }}" method="GET" class="flex gap-2">
        <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari kategori..." class="px-4 py-2 border rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500">
    ...
    <a href="{{ route('admin.categories.create') }}" class="inline-block px-6 py-3
...
<div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
    <table class="w-full text-left border-collapse">
        ...
        <tr class="hover:bg-slate-50/50 transition">
            ...
            <td class="px-8 py-6"> {{ $category->name }} </td>
            <td class="px-8 py-6"> {{ $category->slug }} </td>
            <td class="px-8 py-6 flex gap-2">
                <a href="{{ route('admin.categories.edit', $category->id) }}" class="px-3 py-1 bg-yellow-100 text-yellow-700 rounded-lg text-sm font-bold hover:bg-yellow-200 transition">Edit</a>
