...
@section('page_title', 'Tambah Parner Baru')
...
<div class="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm max-w-3xl">
    ...
    <form action="{{ url('/admin/partners') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        ...
        <div>
            ...
            <input type="text" name="logo_url" value="{{ old('logo_url') }}" class="w-full px-5 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-600 outline-none transition font-medium" required>

            @error('logo_url') <span class="text-red-500 text-sm mt-1">{{ $message }}</span> 
            @enderror
        </div>
