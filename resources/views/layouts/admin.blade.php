    ...
    <aside class="w-64 bg-indigo-900 text-indigo-100 flex flex-col p-6 space-y-8 sticky top-0 h-screen">
        ...
        <nav class="flex-1 space-y-2">
            ...
            @if($role === 'organizer')
                ...
                <a href="{{ route('admin.events.index') }}" class="flex items-center gap-3 px-4 py-3 {{ request()->routeIs('admin.events.index') || (request()->routeIs('admin.events.*') && !request()->routeIs('admin.events.create')) ? 'bg-indigo-800 text-white' : 'hover:bg-indigo-800' }} rounded-xl font-bold transition">
                    <svg class="w-5 h-5 {{ request()->routeIs('admin.events.index') || (request()->routeIs('admin.events.*') && !request()->routeIs('admin.events.create')) ? 'text-indigo-300' : 'text-indigo-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
